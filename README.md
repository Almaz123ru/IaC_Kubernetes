...
## Проблема или фича с выключенным пользователем
После создания машины с помощью terraform, из-за отсутствия полей
```sh
ciuser      = "ubuntu"  # ваш пользователь
cipassword  = "1234"    # пароль для него (разумеется лучше отдельной переменой в .tfvars)
```
в `main.tf`, пользователь по умолчания блокируется Proxmox'м, но сказать что это плохо - нельзя.